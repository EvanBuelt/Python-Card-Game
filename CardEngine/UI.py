...
# Event Handler is used to send data to any function that is subscribed to the particular Event Handler.
# Used in engine for various pygame events.
class EventHandler:
    def __init__(self):
        self._functions = []

    # Add function if function is not already in list using += operator.
    def __iadd__(self, function):
        if function not in self._functions:
            self._functions.append(function)
        return self

    # Removes function from Event Handler using -= operator.
    def __isub__(self, function):
        if function in self._functions:
            self._functions.remove(function)
        return self

    # Add function if function is not registered in Event Handler.
    def append(self, function):
        if function not in self._functions:
            self._functions.append(function)

    # Removes function from the end of the list.  If an index is given, remove given index.
    ...
    # Send the data received to all the functions subscribed to the Event Handler
...
class PyText(UIElement):
    def __init__(self, engine, rect=None, text='',
...
class PyTextBox(UIElement):
    def __init__(self, engine, rect=None, background_text=None, background_color=WHITE,
                 input_text_color=BLACK, background_text_color=LIGHTGRAY,
                 font=None, callback_function=None):
...
class PyCheckBox(UIElement):
    def __init__(self, engine, rect=None, background_color=WHITE, callback_function=None):
